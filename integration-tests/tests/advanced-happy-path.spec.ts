...
describe('Advance Happy path', () => {
  ...
  describe('Trigger a new Pipelinerun related to push event', () => {
    it('Merge the auto-generated PR, and verify the event status on modal', () => {
      ...
      latestCommitsTabPage.mergePR(repoOwner, repoName, 1, componentInfo.get('firstCommitTitle'), componentInfo.get('firstCommitMessage'));
  ...
  describe('Verify and wait for "Pipeline runs" Tab', () => {
    it('Verify the Pipeline run details and Task runs', () => {
      ...
      UIhelper.getTableRow('Pipeline run List', 'Running').contains(`${componentName}-on-push`).invoke('text').then((pipelinerunName) => {
        ...
        DetailsTab.checkStatusSucceeded(TaskRunsTab.getAdvancedTaskNamesList(componentInfo.get('firstPipelineRunName')));
      })
    });
  });
  ...
  describe('Add a new commit and verify changes in the Route', () => {
    ...
    it('Verify and wait for the new Pipeline run', () => {
      ...
      UIhelper.getTableRow('Pipeline run List', 'Running').contains(`${componentName}-on-push`).invoke('text').then((pipelinerunName) => {
        ...
        PipelinerunsTabPage.clickOnRunningPipelinerun(componentName);
        DetailsTab.waitUntilStatusIsNotRunning();
        DetailsTab.checkStatusSucceeded(TaskRunsTab.getAdvancedTaskNamesList(componentInfo.get('secondPipelineRunName')));
      })
    });
  ...
  describe('Explore "Latest commits" Tab', () => {
    it('Verify the Commits List view should have both the commits', () => {
      ...
      UIhelper.verifyRowInTable('Commit List', componentInfo.get('firstCommitTitle'), ['main', componentName, 'hac-test', 'Succeeded'])
      UIhelper.verifyRowInTable('Commit List', componentInfo.get('updatedCommitMessage'), ['main', componentName, 'hac-test', 'Succeeded'])
    });

    it('Verify the Commit Overview Tab of the Last Commit', () => {
      UIhelper.clickRowCellInTable('Commit List', componentInfo.get('updatedCommitMessage'), componentInfo.get('updatedCommitMessage'))
      latestCommitsTabPage.verifyCommitsPageTitleAndStatus(componentInfo.get('updatedCommitMessage'))
      latestCommitsTabPage.verifyCommitID(Cypress.env(`${componentInfo.get('updatedCommitMessage')}_SHA`), repoLink) // Commit SHA was stored in dynamic env at latestCommitsTabPage.editFile()
      latestCommitsTabPage.verifyBranch('main', repoLink)
      latestCommitsTabPage.verifyNodesOnCommitOverview(['commit', `build-${componentName}`, 'development'])
    })

    it('verify the Commit Pipeline runs Tab', () => {
      UIhelper.clickTab('Pipeline runs')
      UIhelper.verifyRowInTable('Pipelinerun List', componentInfo.get('secondPipelineRunName'), ['Succeeded', 'Build'])
