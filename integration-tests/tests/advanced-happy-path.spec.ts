...
describe('Create Components using the UI', () => {
  ...
  componentInfo.set('goFileBase64Original', '');
  componentInfo.set('goFileBase64Updated', '');
  ...
  componentInfo.set('commitMessageRevert', 'Revert: Changing the string value inside HelloServer func');
  ...
  after(function () {
    ...
    latestCommitsTabPage.editFile(
      repoLink, componentInfo.get('filePath'),
  ...
  describe('Trigger a new Pipelinerun related to push event', () => {
    ...
    it('Check the component', () => {
      Applications.checkComponentInListView(componentName, applicationName, 'Build Running', 'Custom build');
    })
  });
  ...
  describe('Explore "Latest commits" Tab', () => {
    it("Verify the Commits List view", () => {
      ...
      for (let commitObject of latestCommitsTabPage.commitList) {
        Common.checkRowValues(commitObject.sha, [commitObject.title, "main", componentName, commitObject.user, "Succeeded"]);
      }
    });
  });
  ...
  describe('Check Component Deployment', () => {
    it("Verify the status code and response body of the deployment URL of each component", () => {
      ...
      cy.get(applicationDetailPagePO.route(componentName), { timeout: 240000 }).invoke('text').then(route => {
        Common.checkResponseBodyAndStatusCode(route, componentInfo.get('deploymentBodyOriginal'), 2000);
      });
  ...
  describe('Add a new commit and verify changes in the Route', () => {
    ...
    it("Check the new Pipeline run", () => {
      ...
      cy.get('tbody tr', { timeout: 20000 }).eq(0).then(($row) => {
    ...
    it("Verify that the component deployment reflects latest changes", () => {
      ...
      cy.get(applicationDetailPagePO.route(componentName), { timeout: 240000 }).invoke('text').then(route => {
        Common.checkResponseBodyAndStatusCode(route, componentInfo.get('deploymentBodyUpdated'), 30000);
      });
