...
describe('Create Component from Public Git Source', () => {
  ...
  const iteration = Cypress.env('PERIODIC_RUN') ? 2 : 1;

  Cypress._.times(iteration).forEach(() => {
    describe('create-application-from-public-git-source', () => {
      before(() => {
      ...
      describe('Verify pipeline runs and latest Commits Count', () => {
        it('Verify pipeline runs', () => {
          Applications.goToPipelinerunsTab();
          UIhelper.getTableRows('Pipeline run List').should('have.length', '1');
        });

        it('Verify Latest Commit List', () => {
          Applications.goToLatestCommitsTab();
          UIhelper.getTableRows('Commit List').should('have.length', '1');
        });
      });
    });
  });
