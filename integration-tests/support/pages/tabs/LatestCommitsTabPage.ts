...
export class LatestCommitsTabPage {
  ...
  fetchCommitSHAOnPR(componentName: string, publicGitRepo: string) {
    ...
    cy.get('@pullNumber').then((pullNumber) => {
      ...
      https://api.github.com/repos/${this.owner}/${this.repoName}/pulls/${pullNumber}/commits`,
      ).then((result) => {
        if (result.code) {
  ...
  mergePR(componentName: string, publicGitRepo: string) {
    ...
    cy.get('@pullNumber').then((pullNumber) => {
      ...
      -d '{"commit_title":"${commitTitle}","commit_message":"${commitMessage}"}'`,
      ).then((result) => {
        if (result.code) {
          throw new Error(`Execution failed \n
          Exit code: ${result.code}
  ...
  editFile(
    ...
    -d '{"message":"${commitMessage}","content":"${updatedFileContentInBase64}","sha":"${fileSHA}"}'`,
    ).then((result) => {
      if (result.code) {
        throw new Error(`Execution failed \n
          Exit code: ${result.code}
