...
describe('PermissionsTable', () => {
  it('should render permissions table', () => {
    ...
    const wsRow = rows.children[0];
    expect(wsRow.children[0]).toHaveTextContent('Workspace');
    expect(wsRow.children[1]).toHaveTextContent('Read');
    const appRow = rows.children[1];
  ...
  it('should show correct permissions for given role', () => {
    ...
    let wsRow = rows.children[0];
    expect(wsRow.children[0]).toHaveTextContent('Workspace');
    expect(wsRow.children[1]).toHaveTextContent('Update');
    let appRow = rows.children[1];
    expect(appRow.children[0]).toHaveTextContent('Application');
    ...
    expect(wsRow.children[0]).toHaveTextContent('Workspace');
    expect(wsRow.children[1]).toHaveTextContent('Everything');
    appRow = rows.children[1];
    expect(appRow.children[0]).toHaveTextContent('Application');
  ...
  it('should correctly format multiple permissions', () => {
    ...
    const appRow = rows.children[3];
    expect(appRow.children[1]).toHaveTextContent('Create, Update and Delete');
  });
});
