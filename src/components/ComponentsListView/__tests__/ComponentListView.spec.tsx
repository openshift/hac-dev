...
describe('ComponentListViewPage', () => {
  ...
  it('should filter cards by type', async () => {
    ...
    const successCb = screen.getByLabelText(/successful/i, {
      selector: 'input',
    }) as HTMLInputElement;
    ...
    const failedCb = screen.getByLabelText(/failed/i, {
      selector: 'input',
    }) as HTMLInputElement;
    ...
    const buildingCb = screen.getByLabelText(/Building/i, {
      selector: 'input',
    }) as HTMLInputElement;
    ...
    expect(screen.queryAllByTestId('component-list-item')).toHaveLength(2);
  });
