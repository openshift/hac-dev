...
type AuthTokenModalProps = ComponentProps & {
  tokenUploadUrl: string;
};

export const AuthTokenModal: React.FC<AuthTokenModalProps> = ({ onClose, tokenUploadUrl }) => {
  ...
  const submitToken = React.useCallback(
    async (values: FormikValues) => {
      try {
        await uploadToken(tokenUploadUrl, values.username, values.token);
        onClose();
      } catch (e) {
        setError(e);
      }
    },
    [tokenUploadUrl, onClose, uploadToken],
  );
