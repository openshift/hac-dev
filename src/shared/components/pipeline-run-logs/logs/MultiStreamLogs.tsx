...
export const MultiStreamLogs: React.FC<MultiStreamLogsProps> = ({
  ...
  const divider = <FlexItem className="hacDev-multi-stream-logs__divider">|</FlexItem>;
  return (
    <div ref={fullscreenRef} className="hacDev-multi-stream-logs">
      <Flex
        className={(classNames as any)({
          'hacDev-multi-stream-logs--fullscreen': isFullscreen,
        })}
      >
        <FlexItem className="hacDev-multi-stream-logs__button" align={{ default: 'alignRight' }}>
          <Button variant="link" onClick={downloadLogs} isInline>
            <DownloadIcon className="hacDev-multi-stream-logs__icon" />
            {t('Download')}
          </Button>
        </FlexItem>
        {divider}
        {onDownloadAll && (
          <>
            <FlexItem className="hacDev-multi-stream-logs__button">
              <Button
                variant="link"
        ...
        {fullscreenToggle && (
          <FlexItem className="hacDev-multi-stream-logs__button">
            <Button variant="link" onClick={fullscreenToggle} isInline>
              {isFullscreen ? (
                <>
                  <CompressIcon className="hacDev-multi-stream-logs__icon" />
                  {t('Collapse')}
                </>
              ) : (
                <>
                  <ExpandIcon className="hacDev-multi-stream-logs__icon" />
                  {t('Expand')}
                </>
              )}
            </Button>
      ...
      <div className="hacDev-multi-stream-logs__taskName">
        {taskName}
        {(loadingContainers || stillFetching) && (
          <span className="hacDev-multi-stream-logs__taskName__loading-indicator">
            <LoadingInline />
          </span>
        )}
      </div>
      <div
        className="hacDev-multi-stream-logs__container"
        onScroll={handleScrollCallback}
        data-test-id="logs-task-container"
      >
        <div className="hacDev-multi-stream-logs__container__logs" ref={scrollPane}>
          {!loadingContainers &&
