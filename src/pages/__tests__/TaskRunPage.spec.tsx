...
jest.mock('../../components/TaskRunDetailsView/TaskRunDetailsView', () => () => (
  <div data-test="taskrun-details-page" />
));
...
describe('TaskRunPage', () => {
  beforeEach(() => {
    useParamsMock.mockReturnValue({ trName: 'my-taskrun' });
  ...
  it('should show the spinner when access check is not loaded', () => {
    ...
    namespaceRenderer(<TaskRunPage />, 'test-ns', { workspacesLoaded: true });

    screen.getByTestId('spinner');
  });

  it('should render taskrun details page', () => {
    accessReviewMock.mockReturnValue([true, true]);
    watchResourceMock.mockReturnValue([[], true, null]);
    namespaceRenderer(<TaskRunPage />, 'test-ns', { workspacesLoaded: true });

    screen.getByTestId('taskrun-details-page');
  });
});
