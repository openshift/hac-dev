...
import { Button, PageSection, TextInput, Title, Alert } from '@patternfly/react-core';
import { getWsTopic } from '../dynamic-plugin-sdk';
...
const TestK8s: React.FC = () => {
  ...
  const [wsPath, setWsPath] = React.useState<string>(null);
  const [rWs, setRWs] = React.useState(null);
    ...
    <React.Fragment>
      <PageSection>
        ...
        <Title headingLevel="h2" size="xl">
          Websockets {namespace === 'default' && <>Needs different namespace than default</>}
        </Title>
        {!wsPath &&
          (namespace ? (
            <>
              ...
              <Button
                isDisabled={namespace === 'default'}
                onClick={() => {
                  setWsPath(getK8sResourceURL(ApplicationModel, undefined, { ns: namespace }));
                }}
                variant="primary"
              >
                Applications
        ...
        {isOpen && !rWs && <p>No response -- did you create the Application?</p>}
        {error && (
          <Alert variant="danger" title="Websocket Error">
            {error}
          </Alert>
        )}
        {rWs && <pre>{JSON.stringify(rWs, null, 2)}</pre>}
      </PageSection>
    </React.Fragment>
  );
};
